...
import com.goldenasia.lottery.util.MemoryManager;
import com.goldenasia.lottery.util.PromptManager;
import com.goldenasia.lottery.util.SoftMap;
 ...
 * @author Ace
 */
public class HallMiddleManager extends Observable {
	private static final String TAG = "MiddleManager";
	private static HallMiddleManager instance = new HallMiddleManager();

	private HallMiddleManager() {
	}

	public static HallMiddleManager getInstance() {
		return instance;
	}
	...
	public boolean goBack() {
		...
		if (HISTORY.size() > 0) {
			...
			if (HISTORY.size() == 1) {
				...
				if(key.equals("PlaySSC")||key.equals("PlaySYFive"))
					changeUI(LotteryHomeHall.class);
				return false;
			}
